 ...
 const { isMacOSApp } = require('./autofill-utils')
 ...
 <div class="wrapper">
     <div class="tooltip" hidden>
         ...
         this.link = shadow.querySelector('style')
 ...
 },{"./autofill-utils":6,"./logo-svg":8}],3:[function(require,module,exports){
 "use strict";
 ...
 var ExtensionInterface = {
   isDeviceSignedIn: function isDeviceSignedIn() {
     return new Promise(function (resolve) {
       return chrome.runtime.sendMessage({
         ...
         return resolve(!!(userData && userData.nextAlias));
       });
     });
   },
   trySigningIn: function trySigningIn() {
     if (isDDGDomain()) {
       sendAndWaitForAnswer(SIGN_IN_MSG, 'addUserData').then(function (data) {
         return DeviceInterface.storeUserData(data);
       });
     }
   },
   ...
   addDeviceListeners: function addDeviceListeners() {
     ...
     chrome.runtime.onMessage.addListener(function (message, sender) {
       ...
       switch (message.type) {
         ...
         case 'contextualAutofill':
           ...
           activeEl.classList.add('ddg-autofilled'); // If the user changes the alias, remove the decoration
   ...
   addLogoutListener: function addLogoutListener(handler) {
   ...
   attachTooltip: function attachTooltip(form, input) {
 ...
 var AndroidInterface = {
   ...
   storeUserData: function storeUserData(_ref) {
     var _ref$addUserData = _ref.addUserData,
         token = _ref$addUserData.token,
         userName = _ref$addUserData.userName;
     return window.EmailInterface.storeCredentials(token, userName);
   },
   addDeviceListeners: function addDeviceListeners() {},
   addLogoutListener: function addLogoutListener() {},
   attachTooltip: function attachTooltip(form, input) {
     form.activeInput = input;
     sendAndWaitForAnswer(function () {
       return window.EmailInterface.showTooltip();
     }, 'getAliasResponse').then(function (res) {
       if (res.alias) form.autofill(res.alias);else form.activeInput.focus();
     });
   }
 };
 var iOSInterface = {
   ...
   storeUserData: function storeUserData(_ref2) {
     var _ref2$addUserData = _ref2.addUserData,
         token = _ref2$addUserData.token,
         userName = _ref2$addUserData.userName;
 ...
 var DeviceInterface = !isDDGApp ? ExtensionInterface : isAndroid ? AndroidInterface : isMacOSApp ? macOSInterface : iOSInterface;
 ...
 },{"./DeviceInterface":3,"./requestIdleCallback":9,"./scanForInputs.js":10,"intersection-observer":1}],8:[function(require,module,exports){
 "use strict";
