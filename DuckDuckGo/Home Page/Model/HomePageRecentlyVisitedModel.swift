...
extension HomePage.Models {
    ...
    final class RecentlyVisitedSiteModel: ObservableObject {
        ...
        func addPage(fromHistory entry: HistoryEntry, bookmarkManager _: BookmarkManager = LocalBookmarkManager.shared) {
...
extension ContentBlocking {
    ...
    func prevalenceForEntity(named entityName: String) -> Double {
        trackerDataManager.trackerData.entities[entityName]?.prevalence ?? 0.0
    }

    func displayNameForEntity(named entityName: String) -> String {
        trackerDataManager.trackerData.entities[entityName]?.displayName ?? entityName
    }
...
extension String {
    ...
    func slugfiscated() -> String {
        let lower = NSMutableString(string: lowercased())
