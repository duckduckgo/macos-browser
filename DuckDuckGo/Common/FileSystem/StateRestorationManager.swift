...
final class StateRestorationManager {
    ...
    private func write(_ data: Data, sync: Bool) {
        ...
        job = DispatchWorkItem {
            do {
                try self.fileStore.persist(data, fileName: self.fileName)
                self.error = nil
            } catch {
                os_log("StateRestorationManager.write failed: %s", "\(error)")
                self.error = error
            }
        }
    ...
    func restoreState(using restore: @escaping (NSCoder) throws -> Void) throws {
        let data = try fileStore.loadData(named: fileName)
