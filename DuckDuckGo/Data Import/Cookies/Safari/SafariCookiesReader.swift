...
final class BinaryReader {
    ...
    func readIntBE(offset: Int) -> UInt32 {
        ...
        let out: Int32 = data.withUnsafeBytes { $0.load(as: Int32.self) }
...
public class BinaryCookies {
    class func parse(cookiePath: String, callback: (BinaryCookiesError?, [Cookie]?) -> Void) throws {
        let parser = CookieParser()
        let data = try Data(contentsOf: URL(fileURLWithPath: cookiePath))
    ...
    class func parse(cookieURL: URL, callback: (BinaryCookiesError?, [Cookie]?) -> Void) throws {
        let parser = CookieParser()
    ...
    class func parse(data: Data, callback: (BinaryCookiesError?, [Cookie]?) -> Void) {
        let parser = CookieParser()
