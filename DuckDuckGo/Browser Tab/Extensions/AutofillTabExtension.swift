...
final class AutofillTabExtension: TabExtension {
    ...
    struct Dependencies {
        ...
        @Injected static var vaultManagerProvider: (SecureVaultManagerDelegate) -> AutofillSecureVaultDelegate = { delegate in
    ...
    func attach(to tab: Tab) {
        ...
        tab.userScriptsPublisher.sink { [weak self] userScripts in
            ...
            self.emailManager = Dependencies.emailManagerProvider(self)
            autofillScript.emailDelegate = self.emailManager
            self.vaultManager = Dependencies.vaultManagerProvider(self)
