...
final class NewTabPageTabExtension: NavigationResponder {
    ...
    init(scriptsPublisher: some Publisher<some NewTabPageUserScriptProvider, Never>, webViewPublisher: some Publisher<WKWebView, Never>) {
        ...
        scriptsPublisher.sink { [weak self] scripts in
            ...
            newTabPageUserScript = scripts.newTabPageUserScript
            newTabPageUserScript?.webViews.add(webView)
        }.store(in: &cancellables)
    }
}
...
extension TabExtensions {
    var newTabPage: NewTabPageTabExtensionProtocol? { resolve(NewTabPageTabExtension.self) }
}
