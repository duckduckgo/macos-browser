...
final class TabDataCleaner: NSObject, WKNavigationDelegate {
    ...
    func prepareModelsForCleanup(
        _ models: [TabViewModel],
...
final class Fire {
    ...
    init(
        cacheManager: WebCacheManager = WebCacheManager.shared,
    ...
    func burnDomains(_ domains: Set<String>, completion: (() -> Void)? = nil) {
        ...
        tabsCleaner.prepareModelsForCleanup(modelsToRemove) {
            ...
            group.enter()
            self.burnTabsFrom(
                collectionViewModels: collectionsCleanupInfo,
    ...
    private func burnHistory(of domains: Set<String>, completion: @escaping () -> Void) {
        historyCoordinating.burnDomains(domains, completion: completion)
    }

    // MARK: - Permissions

    private func burnPermissions(completion: @escaping () -> Void) {
        permissionManager.burnPermissions(except: FireproofDomains.shared, completion: completion)
    }

    private func burnPermissions(of domains: Set<String>, completion: @escaping () -> Void) {
        permissionManager.burnPermissions(of: domains, completion: completion)
    }

    // MARK: - Downloads

    private func burnDownloads() {
        downloadListCoordinator.cleanupInactiveDownloads()
    }

    private func burnDownloads(of domains: Set<String>) {
        downloadListCoordinator.cleanupInactiveDownloads(for: domains)
    }

    // MARK: - Favicons

    private func burnFavicons(completion: @escaping () -> Void) {
        faviconManagement.burnExcept(
    ...
    private func burnTabsFrom(
        collectionViewModels: TabCollectionsCleanupInfo,
...
extension TabCollectionViewModel {
    ...
    fileprivate struct TabCleanupInfo {
        let tabViewModel: TabViewModel
        let action: Tab.FireAction
    }

    fileprivate func prepareCleanupInfoForTabs(relatedToDomains domains: Set<String>) -> [TabCleanupInfo] {
    ...
    fileprivate func clearData(_ cleanupInfo: [TabCleanupInfo], forDomains domains: Set<String>) {
        ...
        if
            let lastRemovedTabHost = tabCollection.lastRemovedTabCache?.url?.host,
...
extension Tab {
    ...
    fileprivate func fireAction(for domains: Set<String>) -> FireAction {
        ...
        if
            localHistory.contains(where: { visitedDomain in
...
extension History {
    ...
    var visitedDomains: Set<String> {
        reduce(Set<String>(), { result, historyEntry in
