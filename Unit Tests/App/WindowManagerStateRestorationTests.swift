...
class WindowManagerStateRestorationTests: XCTestCase {
    ...
    func isTab(_ a: Tab, equalTo b: Tab) -> Bool {
        a.url == b.url
            ...
            && a.maybeWebView?.configuration.websiteDataStore.isPersistent == b.maybeWebView?.configuration.websiteDataStore.isPersistent
    }
    ...
    func testWindowManagerStateRestoration() throws {
        ...
        WindowsManager.shared.openNewWindow(with: model1)
        WindowsManager.shared.openNewWindow(with: model2)
