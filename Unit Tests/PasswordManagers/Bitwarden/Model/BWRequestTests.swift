...
final class BWRequestTests: XCTestCase {
    ...
    func testWhenEncryptedCommandRequestIsMade_ThenItMustContainEncryptedCommandAndMessageId() {
        let encryptedCommand = "12345"
        let messageId = "54321"
        let request = BWRequest.makeEncryptedCommandRequest(encryptedCommand: encryptedCommand, messageId: messageId).data

        XCTAssert(request!.utf8String()!.contains(encryptedCommand))
        XCTAssert(request!.utf8String()!.contains(messageId))
    }
